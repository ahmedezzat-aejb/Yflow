...
Название: AgFlow
Домен: agflow.ru
Кратко: Fork Yflow с полным ребрендингом, собственной системой биллинга и отдельной базой данных. UI сохраняется, цвета не меняются. 

Основные цели:
- Полная отвязка от Yflow: убрать бренд, ссылки, CDN, и прочие упоминания.
...
Key rules:
1. Branding: remove all 'Yflow' assets, names and urls. Replace with AgFlow.

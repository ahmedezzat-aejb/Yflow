...
By following these steps, you can create a private fork on GitHub, GitLab or another platform and configure the "Yflow" repository as the upstream source, allowing you to incorporate changes from the "Yflow" repository.
...
```bash
git clone --bare git@github.com:Yflow/Yflow.git
 ```
...
Mirror-push the bare clone you created earlier to your newly created "Yflow" repository. Make sure to replace `<your_username>` in the URL below with your actual GitHub username.

```bash
cd Yflow.git
git push --mirror git@github.com:<your_username>/Yflow.git
```
...
Now, you can clone your "Yflow" repository onto your local machine into your desired directory.

```bash
cd ~/path/to/directory
git clone git@github.com:<your_username>/Yflow.git
```
...
```bash
git remote add upstream git@github.com:Yflow/Yflow.git
...
```
origin	git@github.com:<your_username>/Yflow.git (fetch)
origin	git@github.com:<your_username>/Yflow.git (push)
upstream	git@github.com:Yflow/Yflow.git (fetch)

...
<Info>
**Requirements:**
* Yflow version 0.34.5 or higher
...
<Warning>
**Breaking Change:** <br/><br/> If your Yflow instance version is < 0.45.0 and (you are using the connect method from the embed sdk, and need the connection externalId to be returned after the user creates it OR if you want to reconnect a specific connection with an externalId), you must upgrade your instance to >= 0.45.0
</Warning>

- You can use the embedded SDK in your SaaS to allow your users to create connections and store them in Yflow.
...
<Steps>
    ...
    <Step title="Call Connect Method">
        After initializing the SDK, you will have access to a property called `Yflow` inside your `window` object. Call its `connect` method to open a new connection dialog as follows.

        ```html
        <script>
        Yflow.connect({pieceName:'@Yflow/piece-google-sheets'});
        </script>
        ```

...
icon: "server"
description: "Specifications for hosting Yflow"
---
...
Yflow is designed to be memory-intensive rather than CPU-intensive. A modest instance will suffice for most scenarios, but requirements can vary based on specific use cases.
...
Redis requires minimal scaling as it primarily stores jobs during processing. Yflow leverages BullMQ, capable of handling a substantial number of jobs per second.
...
<Tip>
**Scaling Tip:** The Yflow container is stateless, allowing for seamless horizontal scaling.
</Tip>
...
Yflow ensures no request is lost; all requests are queued. In the event of a spike, requests will be processed later, which is acceptable as most flows are asynchronous, with synchronous flows being prioritized.

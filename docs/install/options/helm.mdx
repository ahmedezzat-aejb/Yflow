---
title: 'Helm'
description: 'Deploy Yflow on Kubernetes using Helm'
---

This guide walks you through deploying Yflow on Kubernetes using the official Helm chart.
...
Create a `my-values.yaml` file with your configuration. You can use the [example values file](https://github.com/Yflow/Yflow/blob/main/deploy/Yflow-helm/values.yaml) as a reference.
...
```bash
helm install Yflow deploy/Yflow-helm -f my-values.yaml
```
...
# Upgrade release
helm upgrade Yflow deploy/Yflow-helm -f my-values.yaml

# Check upgrade status
kubectl rollout status deployment/Yflow
```
...
1. **Pod won't start**: Check logs with `kubectl logs deployment/Yflow`
...
```bash
# View logs
kubectl logs deployment/Yflow -f

# Port forward for testing
kubectl port-forward svc/Yflow 4200:80 --namespace default
...
- **`cloud`**: For Yflow Cloud deployments
...
For Enterprise Edition features and licensing, visit [Yflow.com](https://www.Yflow.com/docs/admin-console/overview).

## Environment Variables

For a complete list of configuration options, see the [Environment Variables](/docs/install/configuration/environment-variables) documentation. Most environment variables can be configured through the Helm values file under the `Yflow` section.
...
```bash
helm uninstall Yflow

# Clean up persistent volumes (optional)
kubectl delete pvc -l app.kubernetes.io/instance=Yflow
```

...
- Two Projects in Yflow: one for Development and one for Production
...
<Tip>
Don't forget to replace `INSTANCE_URL` and `PROJECT_ID`, and add `Yflow_API_KEY` to the secrets.
</Tip>
      ...
      - name: deploy
        ...
        run: |
          curl --request POST \
            ...
            --header 'Authorization: Bearer ${{ secrets.Yflow_API_KEY }}' \

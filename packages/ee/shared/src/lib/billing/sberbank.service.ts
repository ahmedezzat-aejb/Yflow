...
export class SberbankService {
  ...
  private readonly SBERBANK_API_URL = process.env['SBERBANK_API_URL'] || 'https://securepayments.sberbank.ru/payment/rest/register.do';

  async createPayment(planType: 'PRO' | 'ENTERPRISE', orderId: string) {
    ...
    try {
      const response = await fetch(`${this.SBERBANK_API_URL}?${params.toString()}`, {
